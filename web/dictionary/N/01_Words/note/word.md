# WORD: Note

## Meaning (plain):
A word that signals weight, momentum, or authority.

## Rap meaning (how I'd say it):
Used to signal strength, control, or momentum in a bar.

## Syllables:
1 (approx.)

## Part of speech:
Noun/Adjective

## Synonyms:
drive, edge, authority, control, energy, weight

## Antonyms:
surrender, silence, fragility, doubt

## Rhyme ideas (end / slant / multi):
END: ote, oted, otes / SLANT: ot, otee, otey / MULTI: note mode, note codes, note road

## 3 bar-ready examples:
1. Say Note with the vision, watch the crowd awake.
2. Note in my cadence, heavy like a drumline.
3. I move with Note, every step a headline.

## 3 punchline angles:
1. Force/weight contrast
2. Power/control imagery
3. Scale/impact flex

## Tags:
power, intensity, dominance, momentum