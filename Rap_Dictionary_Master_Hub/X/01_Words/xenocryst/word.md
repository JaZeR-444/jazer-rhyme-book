# WORD: Xenocryst

## Meaning (plain):
A word that signals weight, momentum, or authority.

## Rap meaning (how I'd say it):
A punchy term for dominance, pressure, and presence.

## Syllables:
3 (approx.)

## Part of speech:
Noun/Adjective

## Synonyms:
energy, weight, power, force, strength, might

## Antonyms:
hesitation, decline, weakness, softness

## Rhyme ideas (end / slant / multi):
END: yst, ystd, ysts / SLANT: ys, yste, ysty / MULTI: xenocryst mode, xenocryst codes, xenocryst road

## 3 bar-ready examples:
1. Xenocryst in the flow, make the speakers quake.
2. Say Xenocryst with the vision, watch the crowd awake.
3. Xenocryst in my cadence, heavy like a drumline.

## 3 punchline angles:
1. Leadership/crown imagery
2. Force/weight contrast
3. Power/control imagery

## Tags:
power, intensity, dominance, momentum