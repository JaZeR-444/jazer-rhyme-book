...
const SOURCE_DICT = path.join(__dirname, '../Rap_Dictionary_Master_Hub');
const DEST_PUBLIC_DATA = path.join(__dirname, '../web/public/data');
...
const domains = fs.readdirSync(SOURCE_DATA).filter(item => {
    return fs.statSync(path.join(SOURCE_DATA, item)).isDirectory() && !item.startsWith('_');
});
