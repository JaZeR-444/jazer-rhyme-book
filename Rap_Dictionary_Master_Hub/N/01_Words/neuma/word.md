# WORD: Neuma

## Meaning (plain):
A bold term associated with strength, scale, or force.

## Rap meaning (how I'd say it):
A punchy term for dominance, pressure, and presence.

## Syllables:
2 (approx.)

## Part of speech:
Noun/Adjective

## Synonyms:
edge, authority, control, energy, weight, power

## Antonyms:
silence, fragility, doubt, hesitation

## Rhyme ideas (end / slant / multi):
END: uma, umad, umas / SLANT: um, umae, umay / MULTI: neuma mode, neuma codes, neuma road

## 3 bar-ready examples:
1. Say Neuma with the vision, watch the crowd awake.
2. Neuma in my cadence, heavy like a drumline.
3. I move with Neuma, every step a headline.

## 3 punchline angles:
1. Force/weight contrast
2. Power/control imagery
3. Scale/impact flex

## Tags:
power, intensity, dominance, momentum