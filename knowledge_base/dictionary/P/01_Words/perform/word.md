# WORD: Perform

## Meaning (plain):
A word that signals weight, momentum, or authority.

## Rap meaning (how I'd say it):
Signals big energy, authority, and heavy impact.

## Syllables:
2 (approx.)

## Part of speech:
Noun/Adjective

## Synonyms:
strength, might, impact, pressure, drive, edge

## Antonyms:
surrender, silence, fragility, doubt

## Rhyme ideas (end / slant / multi):
END: orm, ormd, orms / SLANT: or, orme, ormy / MULTI: perform mode, perform codes, perform road

## 3 bar-ready examples:
1. Perform in my cadence, heavy like a drumline.
2. I move with Perform, every step a headline.
3. Perform on the mic, watch the room align.

## 3 punchline angles:
1. Power/control imagery
2. Scale/impact flex
3. Momentum/pressure metaphor

## Tags:
power, intensity, dominance, momentum