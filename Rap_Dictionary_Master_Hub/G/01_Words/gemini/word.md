# WORD: Gemini

## Meaning (plain):
A forceful term evoking power, impact, or presence.

## Rap meaning (how I'd say it):
A punchy term for dominance, pressure, and presence.

## Syllables:
3 (approx.)

## Part of speech:
Noun/Adjective

## Synonyms:
impact, pressure, drive, edge, authority, control

## Antonyms:
fragility, doubt, hesitation, decline

## Rhyme ideas (end / slant / multi):
END: ini, inid, inis / SLANT: in, inie, iniy / MULTI: gemini mode, gemini codes, gemini road

## 3 bar-ready examples:
1. Say Gemini with the vision, watch the crowd awake.
2. Gemini in my cadence, heavy like a drumline.
3. I move with Gemini, every step a headline.

## 3 punchline angles:
1. Force/weight contrast
2. Power/control imagery
3. Scale/impact flex

## Tags:
power, intensity, dominance, momentum