# WORD: Flags

## Meaning (plain):
A word that signals weight, momentum, or authority.

## Rap meaning (how I'd say it):
A punchy term for dominance, pressure, and presence.

## Syllables:
1 (approx.)

## Part of speech:
Noun/Adjective

## Synonyms:
energy, weight, power, force, strength, might

## Antonyms:
surrender, silence, fragility, doubt

## Rhyme ideas (end / slant / multi):
END: ags, agsd, agss / SLANT: ag, agse, agsy / MULTI: flags mode, flags codes, flags road

## 3 bar-ready examples:
1. Say Flags with the vision, watch the crowd awake.
2. Flags in my cadence, heavy like a drumline.
3. I move with Flags, every step a headline.

## 3 punchline angles:
1. Force/weight contrast
2. Power/control imagery
3. Scale/impact flex

## Tags:
power, intensity, dominance, momentum