# WORD: Minimum

## Meaning (plain):
A forceful term evoking power, impact, or presence.

## Rap meaning (how I'd say it):
Signals big energy, authority, and heavy impact.

## Syllables:
3 (approx.)

## Part of speech:
Noun/Adjective

## Synonyms:
authority, control, energy, weight, power, force

## Antonyms:
weakness, softness, surrender, silence

## Rhyme ideas (end / slant / multi):
END: mum, mumd, mums / SLANT: mu, mume, mumy / MULTI: minimum mode, minimum codes, minimum road

## 3 bar-ready examples:
1. Minimum in the flow, make the speakers quake.
2. Say Minimum with the vision, watch the crowd awake.
3. Minimum in my cadence, heavy like a drumline.

## 3 punchline angles:
1. Leadership/crown imagery
2. Force/weight contrast
3. Power/control imagery

## Tags:
power, intensity, dominance, momentum