# WORD: Subsidiary

## Meaning (plain):
A word that signals weight, momentum, or authority.

## Rap meaning (how I'd say it):
Used to signal strength, control, or momentum in a bar.

## Syllables:
4 (approx.)

## Part of speech:
Adjective

## Synonyms:
drive, edge, authority, control, energy, weight

## Antonyms:
surrender, silence, fragility, doubt

## Rhyme ideas (end / slant / multi):
END: ary, aryd, arys / SLANT: ar, arye, aryy / MULTI: subsidiary mode, subsidiary codes, subsidiary road

## 3 bar-ready examples:
1. Subsidiary in the flow, make the speakers quake.
2. Say Subsidiary with the vision, watch the crowd awake.
3. Subsidiary in my cadence, heavy like a drumline.

## 3 punchline angles:
1. Leadership/crown imagery
2. Force/weight contrast
3. Power/control imagery

## Tags:
power, intensity, dominance, momentum