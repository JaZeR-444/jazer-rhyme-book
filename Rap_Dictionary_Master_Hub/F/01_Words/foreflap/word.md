# WORD: Foreflap

## Meaning (plain):
A word that signals weight, momentum, or authority.

## Rap meaning (how I'd say it):
A punchy term for dominance, pressure, and presence.

## Syllables:
3 (approx.)

## Part of speech:
Noun/Adjective

## Synonyms:
energy, weight, power, force, strength, might

## Antonyms:
hesitation, decline, weakness, softness

## Rhyme ideas (end / slant / multi):
END: lap, lapd, laps / SLANT: la, lape, lapy / MULTI: foreflap mode, foreflap codes, foreflap road

## 3 bar-ready examples:
1. Foreflap in my cadence, heavy like a drumline.
2. I move with Foreflap, every step a headline.
3. Foreflap on the mic, watch the room align.

## 3 punchline angles:
1. Power/control imagery
2. Scale/impact flex
3. Momentum/pressure metaphor

## Tags:
power, intensity, dominance, momentum