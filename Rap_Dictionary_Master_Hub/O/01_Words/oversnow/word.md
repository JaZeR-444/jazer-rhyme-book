# WORD: Oversnow

## Meaning (plain):
A word that signals weight, momentum, or authority.

## Rap meaning (how I'd say it):
Signals big energy, authority, and heavy impact.

## Syllables:
3 (approx.)

## Part of speech:
Noun/Adjective

## Synonyms:
strength, might, impact, pressure, drive, edge

## Antonyms:
surrender, silence, fragility, doubt

## Rhyme ideas (end / slant / multi):
END: now, nowd, nows / SLANT: no, nowe, nowy / MULTI: oversnow mode, oversnow codes, oversnow road

## 3 bar-ready examples:
1. Oversnow in the flow, make the speakers quake.
2. Say Oversnow with the vision, watch the crowd awake.
3. Oversnow in my cadence, heavy like a drumline.

## 3 punchline angles:
1. Leadership/crown imagery
2. Force/weight contrast
3. Power/control imagery

## Tags:
power, intensity, dominance, momentum