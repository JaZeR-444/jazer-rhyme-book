# WORD: Then

## Meaning (plain):
A forceful term evoking power, impact, or presence.

## Rap meaning (how I'd say it):
A punchy term for dominance, pressure, and presence.

## Syllables:
1 (approx.)

## Part of speech:
Noun/Adjective

## Synonyms:
impact, pressure, drive, edge, authority, control

## Antonyms:
fragility, doubt, hesitation, decline

## Rhyme ideas (end / slant / multi):
END: hen, hend, hens / SLANT: he, hene, heny / MULTI: then mode, then codes, then road

## 3 bar-ready examples:
1. Say Then with the vision, watch the crowd awake.
2. Then in my cadence, heavy like a drumline.
3. I move with Then, every step a headline.

## 3 punchline angles:
1. Force/weight contrast
2. Power/control imagery
3. Scale/impact flex

## Tags:
power, intensity, dominance, momentum