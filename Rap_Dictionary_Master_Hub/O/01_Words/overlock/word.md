# WORD: Overlock

## Meaning (plain):
A forceful term evoking power, impact, or presence.

## Rap meaning (how I'd say it):
A punchy term for dominance, pressure, and presence.

## Syllables:
3 (approx.)

## Part of speech:
Noun/Adjective

## Synonyms:
impact, pressure, drive, edge, authority, control

## Antonyms:
weakness, softness, surrender, silence

## Rhyme ideas (end / slant / multi):
END: ock, ockd, ocks / SLANT: oc, ocke, ocky / MULTI: overlock mode, overlock codes, overlock road

## 3 bar-ready examples:
1. Say Overlock with the vision, watch the crowd awake.
2. Overlock in my cadence, heavy like a drumline.
3. I move with Overlock, every step a headline.

## 3 punchline angles:
1. Force/weight contrast
2. Power/control imagery
3. Scale/impact flex

## Tags:
power, intensity, dominance, momentum