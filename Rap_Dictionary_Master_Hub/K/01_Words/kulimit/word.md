# WORD: Kulimit

## Meaning (plain):
A forceful term evoking power, impact, or presence.

## Rap meaning (how I'd say it):
Used to signal strength, control, or momentum in a bar.

## Syllables:
3 (approx.)

## Part of speech:
Noun/Adjective

## Synonyms:
power, force, strength, might, impact, pressure

## Antonyms:
fragility, doubt, hesitation, decline

## Rhyme ideas (end / slant / multi):
END: mit, mitd, mits / SLANT: mi, mite, mity / MULTI: kulimit mode, kulimit codes, kulimit road

## 3 bar-ready examples:
1. Kulimit in my cadence, heavy like a drumline.
2. I move with Kulimit, every step a headline.
3. Kulimit on the mic, watch the room align.

## 3 punchline angles:
1. Power/control imagery
2. Scale/impact flex
3. Momentum/pressure metaphor

## Tags:
power, intensity, dominance, momentum