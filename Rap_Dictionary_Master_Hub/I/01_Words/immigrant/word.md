# WORD: Immigrant

## Meaning (plain):
A bold term associated with strength, scale, or force.

## Rap meaning (how I'd say it):
Signals big energy, authority, and heavy impact.

## Syllables:
3 (approx.)

## Part of speech:
Adjective

## Synonyms:
weight, power, force, strength, might, impact

## Antonyms:
silence, fragility, doubt, hesitation

## Rhyme ideas (end / slant / multi):
END: ant, antd, ants / SLANT: an, ante, anty / MULTI: immigrant mode, immigrant codes, immigrant road

## 3 bar-ready examples:
1. Say Immigrant with the vision, watch the crowd awake.
2. Immigrant in my cadence, heavy like a drumline.
3. I move with Immigrant, every step a headline.

## 3 punchline angles:
1. Force/weight contrast
2. Power/control imagery
3. Scale/impact flex

## Tags:
power, intensity, dominance, momentum