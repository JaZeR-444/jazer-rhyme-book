# WORD: Hardware

## Meaning (plain):
A word that signals weight, momentum, or authority.

## Rap meaning (how I'd say it):
A punchy term for dominance, pressure, and presence.

## Syllables:
2 (approx.)

## Part of speech:
Noun/Adjective

## Synonyms:
energy, weight, power, force, strength, might

## Antonyms:
hesitation, decline, weakness, softness

## Rhyme ideas (end / slant / multi):
END: are, ared, ares / SLANT: ar, aree, arey / MULTI: hardware mode, hardware codes, hardware road

## 3 bar-ready examples:
1. Hardware in my cadence, heavy like a drumline.
2. I move with Hardware, every step a headline.
3. Hardware on the mic, watch the room align.

## 3 punchline angles:
1. Power/control imagery
2. Scale/impact flex
3. Momentum/pressure metaphor

## Tags:
power, intensity, dominance, momentum