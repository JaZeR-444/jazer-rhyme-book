# WORD: Tigon

## Meaning (plain):
A bold term associated with strength, scale, or force.

## Rap meaning (how I'd say it):
Signals big energy, authority, and heavy impact.

## Syllables:
2 (approx.)

## Part of speech:
Noun/Adjective

## Synonyms:
weight, power, force, strength, might, impact

## Antonyms:
decline, weakness, softness, surrender

## Rhyme ideas (end / slant / multi):
END: gon, gond, gons / SLANT: go, gone, gony / MULTI: tigon mode, tigon codes, tigon road

## 3 bar-ready examples:
1. Tigon in the flow, make the speakers quake.
2. Say Tigon with the vision, watch the crowd awake.
3. Tigon in my cadence, heavy like a drumline.

## 3 punchline angles:
1. Leadership/crown imagery
2. Force/weight contrast
3. Power/control imagery

## Tags:
power, intensity, dominance, momentum