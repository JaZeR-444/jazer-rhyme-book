# WORD: Fails

## Meaning (plain):
A word that signals weight, momentum, or authority.

## Rap meaning (how I'd say it):
Used to signal strength, control, or momentum in a bar.

## Syllables:
1 (approx.)

## Part of speech:
Noun/Adjective

## Synonyms:
drive, edge, authority, control, energy, weight

## Antonyms:
surrender, silence, fragility, doubt

## Rhyme ideas (end / slant / multi):
END: ils, ilsd, ilss / SLANT: il, ilse, ilsy / MULTI: fails mode, fails codes, fails road

## 3 bar-ready examples:
1. Say Fails with the vision, watch the crowd awake.
2. Fails in my cadence, heavy like a drumline.
3. I move with Fails, every step a headline.

## 3 punchline angles:
1. Force/weight contrast
2. Power/control imagery
3. Scale/impact flex

## Tags:
power, intensity, dominance, momentum