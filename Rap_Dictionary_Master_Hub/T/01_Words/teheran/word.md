# WORD: Teheran

## Meaning (plain):
A bold term associated with strength, scale, or force.

## Rap meaning (how I'd say it):
Signals big energy, authority, and heavy impact.

## Syllables:
3 (approx.)

## Part of speech:
Noun/Adjective

## Synonyms:
weight, power, force, strength, might, impact

## Antonyms:
decline, weakness, softness, surrender

## Rhyme ideas (end / slant / multi):
END: ran, rand, rans / SLANT: ra, rane, rany / MULTI: teheran mode, teheran codes, teheran road

## 3 bar-ready examples:
1. Teheran in my cadence, heavy like a drumline.
2. I move with Teheran, every step a headline.
3. Teheran on the mic, watch the room align.

## 3 punchline angles:
1. Power/control imagery
2. Scale/impact flex
3. Momentum/pressure metaphor

## Tags:
power, intensity, dominance, momentum