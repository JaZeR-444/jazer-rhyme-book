# WORD: Noise

## Meaning (plain):
A word that signals weight, momentum, or authority.

## Rap meaning (how I'd say it):
Signals big energy, authority, and heavy impact.

## Syllables:
1 (approx.)

## Part of speech:
Verb

## Synonyms:
strength, might, impact, pressure, drive, edge

## Antonyms:
surrender, silence, fragility, doubt

## Rhyme ideas (end / slant / multi):
END: ise, ised, ises / SLANT: is, isee, isey / MULTI: noise mode, noise codes, noise road

## 3 bar-ready examples:
1. Say Noise with the vision, watch the crowd awake.
2. Noise in my cadence, heavy like a drumline.
3. I move with Noise, every step a headline.

## 3 punchline angles:
1. Force/weight contrast
2. Power/control imagery
3. Scale/impact flex

## Tags:
power, intensity, dominance, momentum