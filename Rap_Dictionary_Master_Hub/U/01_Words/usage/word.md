# WORD: Usage

## Meaning (plain):
A word that signals weight, momentum, or authority.

## Rap meaning (how I'd say it):
A punchy term for dominance, pressure, and presence.

## Syllables:
2 (approx.)

## Part of speech:
Noun/Adjective

## Synonyms:
energy, weight, power, force, strength, might

## Antonyms:
surrender, silence, fragility, doubt

## Rhyme ideas (end / slant / multi):
END: age, aged, ages / SLANT: ag, agee, agey / MULTI: usage mode, usage codes, usage road

## 3 bar-ready examples:
1. Say Usage with the vision, watch the crowd awake.
2. Usage in my cadence, heavy like a drumline.
3. I move with Usage, every step a headline.

## 3 punchline angles:
1. Force/weight contrast
2. Power/control imagery
3. Scale/impact flex

## Tags:
power, intensity, dominance, momentum